...
{ 
	...
	num=rand() % 8; // rango de valores y realiza la asignacion de la variable num a una variable aleatoria
	printf ("\n----------------------------------------------------"); 
	switch(num)  //menu de seleccion
	{
		
		// me entrega el Descuento de manera aleaotria al azar 
		// es decir me indica si obtuve bola negra roja, verde, o azul de manera aleatoria, al azar como si fueran dados
		// y de esa forma asigna lso descuentos	
		case 0: 
	...
	descuento = (importeCompra - ( importeCompra* descuento / 100)); // asigna el descuento
	printf ("\nEl importe total con el descuento será¡ de: $ %.2f \n\n", descuento); 
	getchar();
	getchar();// detiene la pantalla para que yo pueda ver mi descuento
return 1; 
} 
